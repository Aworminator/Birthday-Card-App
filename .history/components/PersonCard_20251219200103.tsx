...
export default function PersonCard({
  ...
  useEffect(() => {
    ...
    const updateDuration = () => {
      // Handle browsers (e.g., Safari) reporting Infinity/0 duration
      ...
      if (!isFinite(d) || d === 0) {
        const fixDuration = () => {
          if (isFinite(audio.duration) && audio.duration > 0) {
            ...
            // Reset position to start after probing
            try {
              audio.currentTime = 0;
            } catch {}
          }
        };
        // Seek far to force duration calculation, then listen for timeupdate
        audio.addEventListener("timeupdate", fixDuration);
        try {
          const prevPaused = audio.paused;
          audio.currentTime = 1e101;
          // No need to play; seeking should trigger a timeupdate
          if (prevPaused) {
            // keep paused state
          }
        } catch {}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        <audio ref={audioRef} src={card.audio_url} preload="metadata" playsInline />
