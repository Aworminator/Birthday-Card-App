...
export default function AudioUpload({
  ...
  useEffect(() => {
    ...
    const handleCanPlayThrough = () => {
      // Ensure we have enough buffer for first full play
      // If user pressed play before ready, auto-start now
      if (!isPlaying && audio.paused && audio.currentTime === 0) {
        audio
          .play()
          .then(() => {
            // UI will reflect playing state via 'play' event
          })
          .catch(() => {});
      }
    };
  ...
  const handleFileChange = (file: File | null) => {
    if (file && file.type.startsWith("audio/")) {
      ...
      setCurrentTime(0);
      setIsPlaying(false);
      // Force load and buffer to avoid early cutout on first play
      setTimeout(() => {
        const audio = audioRef.current;
        if (audio) {
          audio.preload = "auto";
  ...
  const startRecording = async () => {
    try {
      ...
      if (typeof MediaRecorder === "undefined") {
        alert(
          "Voice recording is not supported on this browser. Please upload an audio file instead."
        );
        return;
      }
      ...
      // Pick a supported mime type and extension per browser
      const tryTypes = [
      ...
      const mimeType =
        tryTypes.find((t) => {
          try {
            ...
            return typeof isSupported === "function" ? isSupported(t) : false;
          } catch {
            return false;
          }
        }) || "";
      ...
      mediaRecorder.onstop = () => {
        ...
        const ext = usedType.includes("mp4")
          ? "mp4"
          : usedType.includes("webm")
          ? "webm"
          : "mp4"; // default to mp4 for broader playback
        const audioBlob = new Blob(chunksRef.current, { type: usedType });
        const audioFile = new File([audioBlob], `voice-memo.${ext}`, {
          type: usedType,
        });
  ...
  const togglePlayPause = () => {
    ...
    if (isPlaying) {
      ...
      // If not sufficiently buffered, wait for canplaythrough, then play
