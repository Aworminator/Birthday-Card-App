...
export default function ProjectPage() {
  ...
  // When music source changes in view mode, ensure element is prepared
  useEffect(() => {
    if (!viewMode) return;
    const el = bgAudioRef.current;
    if (!el) return;
    // Set base element properties; actual volume is controlled by Web Audio when started
    try {
      el.playsInline = true as any;
    } catch {}
    el.muted = true; // prevent duplicate direct output; Web Audio will carry the sound
    try {
      el.volume = 1.0; // element volume ignored on iOS; use GainNode instead
    } catch {}
  ...
  const handleEnterViewMode = () => {
    ...
    // Initialize background music volume to fixed 0.4
    setTimeout(() => {
      ...
      if (audio) {
        ...
        bgAudioHandleRef.current = setupBgAudio(audio, 0.4);
      }
    }, 50);
            ...
            <button
              onClick={async () => {
                ...
                if (isPlaying) {
                  ...
                  try {
                    // Create AudioContext and routing only on user gesture (iOS requirement)
                    ...
                    if (!gainNodeRef.current) {
                      ...
                      gainNodeRef.current.gain.value = 0.4; // default background volume
                    }
                    if (!sourceRef.current) {
                      try {
                        sourceRef.current = ctx.createMediaElementSource(audio);
                      } catch (_) {
                        // If already created previously, ignore
                      }
                    }
                    // Connect graph: element -> gain -> destination
                    try {
                    ...
                    // Ensure inline playback and mute the element's direct output
                    try {
                    ...
                    // Resume context and start playback within the same gesture
