...
import { setupBgAudio, type BgAudioHandle } from "@/lib/bgAudio";
import { useParams, useRouter } from "next/navigation";
import { supabase, BirthdayCard, Project } from "@/lib/supabase";
...
export default function ProjectPage() {
  ...
  const bgAudioHandleRef = useRef<BgAudioHandle | null>(null);
  ...
  // Initialize Web Audio routing on iOS with initial gain, and sync volume
  useEffect(() => {
    if (!viewMode) return;
    const el = bgAudioRef.current;
    if (!el) return;
    // Recreate handle when music source/theme toggles change
    bgAudioHandleRef.current?.destroy?.();
    bgAudioHandleRef.current = setupBgAudio(el, bgVolume);

    const applyVolume = () => {
      el.muted = false;
      try {
        el.volume = Math.max(0, Math.min(1, bgVolume));
      } catch {}
      bgAudioHandleRef.current?.setGain?.(bgVolume);
    };
    const onPlaying = applyVolume;
    el.addEventListener("playing", onPlaying);
    if (!el.paused) applyVolume();
    return () => {
      el.removeEventListener("playing", onPlaying);
    };
  }, [viewMode, customMusicUrl, useDefaultMusic, savedTheme]);

  // Apply gain immediately when slider changes
  useEffect(() => {
    if (!viewMode) return;
    const el = bgAudioRef.current;
    if (!el) return;
  ...
  // Apply volume immediately when slider changes in view mode
  useEffect(() => {
    if (!viewMode) return;
    ...
    if (!audio) return;
    audio.muted = false;
    audio.volume = Math.max(0, Math.min(1, bgVolume));
  }, [bgVolume, viewMode]);
  ...
  const handleEnterViewMode = () => {
    ...
    // Initialize background music volume to slider value
    setTimeout(() => {
      ...
      if (audio) {
        ...
        // Ensure Web Audio is configured with current gain
        bgAudioHandleRef.current?.destroy?.();
        bgAudioHandleRef.current = setupBgAudio(audio, bgVolume);
      }
    }, 50);
      ...
      {viewMode && (
        <>
          <div className="fixed top-4 right-4 z-50 flex flex-wrap gap-2 md:gap-3 max-w-xs md:max-w-none">
            ...
            <button
              onClick={async () => {
                ...
                if (isPlaying) {
                  ...
                  try {
                    await bgAudioHandleRef.current?.resume?.();
                    await audio.play();
                    audio.muted = false;
                    try {
                      audio.volume = Math.max(0, Math.min(1, bgVolume));
            ...
            <div className="flex items-center gap-2 bg-white/80 backdrop-blur px-3 py-2 rounded-xl shadow-md border border-gray-200">
              <svg
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M9 19V6l-2 2H4a1 1 0 00-1 1v6a1 1 0 001 1h3l2 2zm7-2a3 3 0 000-10"
                />
              </svg>
              <input
                type="range"
                ...
                className="w-28 md:w-36 accent-emerald-500"
                aria-label="Background music volume"
              />
              <span className="text-[11px] md:text-xs text-gray-600 font-semibold w-8 text-right">
