...
export default function ProjectPage() {
  ...
  const ensureMobileAudioGraph = (audio: HTMLAudioElement) => {
    if (!isMobileDevice() || !isIOSWebAudioSupported()) return null;
    ...
    if (!sourceRef.current) {
      try {
        ...
        // Creating a second source for the same element throws; ignore
      }
    }
    try {
      if (sourceRef.current) {
        ...
        // Mute element output; route via Web Audio for volume control
        audio.muted = true;
      }
    } catch {
      // No-op
  ...
  const startMusicWithMobileDuck = async () => {
  ...
  // Schedule mobile ducking based on audio playback events
  useEffect(() => {
    if (!viewMode) return;
    const audio = document.getElementById(
      "background-music"
    ...
    const onPlaying = () => {
      clearDuckTimeout();
      // Desktop: use native volume; Mobile: use Web Audio gain
      if (isMobileDevice()) {
        const graph = ensureMobileAudioGraph(audio);
        if (graph) {
    ...
    window.addEventListener("bg-audio-resume", onResumeRequest as EventListener);
    return () => {
      ...
      window.removeEventListener(
        "bg-audio-resume",
        onResumeRequest as EventListener
      );
    };
  ...
  const handleEnterViewMode = () => {
    ...
    // Initialize background music volume to 1.0 (desktop path)
    setTimeout(() => {
      const audio = document.getElementById(
        "background-music"
      ) as HTMLAudioElement;
      if (audio) {
        if (!isMobileDevice()) audio.volume = 1.0;
        // On mobile, prime Web Audio graph during this user gesture
        if (isMobileDevice()) {
          ensureMobileAudioGraph(audio);
        }
      }
    }, 50);
