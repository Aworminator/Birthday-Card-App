...
import { supabase, BirthdayCard, ShareSession } from "@/lib/supabase";
...
export default function SharePage() {
  ...
  useEffect(() => {
    ...
    if (session?.automatic_mode && cards.length > 0) {
      setTimeout(() => {
  ...
  const getMusicUrl = () => {
    if (session?.custom_music_url) return session.custom_music_url;
    if (session?.use_default_music) return getDefaultMusicUrl();
    return null;
  };
