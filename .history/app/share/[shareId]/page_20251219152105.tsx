...
export default function SharePage() {
      ...
      if (isMobileDevice()) {
        ...
        if (graph) {
          ...
          duckTimeoutRef.current = window.setTimeout(() => {
            if (!audio.paused) gain.gain.setValueAtTime(0.4, ctx.currentTime);
          }, 3000);
        }
      } else {
        audio.volume = 1.0;
  ...
  }, [project?.custom_music_url, project?.use_default_music, project?.theme]);
  ...
  useEffect(() => {
    ...
    if (project?.automatic_mode && cards.length > 0) {
      setTimeout(() => {
        ...
        if (audio) {
          // Prepare Web Audio graph early on mobile
          if (isMobileDevice()) ensureMobileAudioGraph(audio);
          audio
            .play()
